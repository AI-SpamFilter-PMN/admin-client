...
public class SubscribersServlet extends HttpServlet {
                ...
                <script>
                  (function() {
                    ...
                    els.body.addEventListener('click', async (e) => {
                      ...
                      if (!ok) return;
                      try {
                        await api('%s', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, status: to }) });
    ...
    private String row(Subscriber s) {
        ...
        return """
                ...
                actions, WebPage.escape(s.getMsisdn()), WebPage.escape(s.getMsisdn()));
    }
